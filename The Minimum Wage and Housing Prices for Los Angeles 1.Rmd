...
A minimum wage raise was recently approved by the city of Los Angeles, CA. The minimum wage will rise to $15 an hour by 2020. The main question I am asking in this project is this: Does the minimum wage increase to $15 an hour by 2020 make monthly rent or mortgage payments more affordable for the minimum wage earner in 2020 compared to the minimum wage earner in 2016? In addition, I will look a little further into this topic. If the wage increase makes the monthly payment for housing more affordable, how affordable is it? Is there any scenario that a monthly payment for a place to live in Los Angeles will be the recommended 30% of the wage earner's budget? Also included in this project is the average wage for all incomes in Los Angeles County for comparison with a minimum wage earner.

## Motivation

My primary client for this project would be the Movement for $15. The Movement for $15 is a nationwide movement for a $15 an hour minimum wage in various cities and states. I chose this client because the data I was able to access allowed me to run an analysis that bests look at the problem from their perspective. I have a future value that is set for a minimum wage in 2020 that I can compare to what this project's predictions tell me. Based off my analysis, the Movement for $15 can decide if the wage they are fighting for is making the impact they want it to. 

The project's audience extends beyond minimum wage earners that are fighting for a higher wage. Those who invest in or work in the housing industry would also be interested in this project. If I were to tailor my analysis to this client, I would want to show in what neighborhoods of Los Angeles there are higher numbers of minimum wage earners. I would also need data that records the price of housing in the specific neighborhood for each year to see if the price of housing is rising at different rates for the neighborhoods. With better predictions for the housing market, the client could decide what kind of housing should be built in a neighborhood and what the expected sale or rent price should be.

Legislators in Los Angeles and other government bodies may also have an interest in this project since they are the one's approving minimum wage increases. If I ran an analysis for legislators, I might want to break down wage earners by specific industries. For instance, they may want to  see what industries have the highest number of minimum wage workers so they can pass legislation that targets those specific industries and provide incentives for employers to raise wages. The city could also take from that analysis that the average rent price may be too high for the amount of minimum wage workers in the city and approve more projects that construct affordable apartments.
...
income2012 <- read.csv("bls data 2012.csv")
...
```
I created a minimum wage dataframe by using the years in which the minimum wage was raised and the values reported on the California Department of Industrial Relations site. I need to compare data with and without the wage raise to $15 an hour. I made two dataframes for minimum wage, one with the assumed wage of $15 in 2020 and one without it.
```{r, echo=TRUE}
...
# Make a second dataframe without the 2020 row. The final dataframe includes just Year and min_monthly_wage_without20 variables.

df_min_wage_without20 <- df_min_wage[-8,]

df_min_wage_without20_final <- data.frame(Year = df_min_wage_without20$Year, min_monthly_wage_without20 = df_min_wage_without20$min_monthly_wage)
```
The average wage data was located in seperate dataframes which recorded values for one year. In addition, I could only base 2016 off the 1st quarter, which when I checked the records, was a good representation of close to 25% of the yearly value. I created an average annual pay column by multiplying the monthly by 12 and then selected and renamed matching columns so I could bind my databases for years 2012 to 2016. I made sure the "year" column is changed to "Year" to match up with my other databases.
```{r, echo=TRUE}
...
# Capitalize year so it is consistent with other dataframes.

names(df_avg_wage) = c("Year", "avg_wkly_wage", "avg_annual_pay", "oty_avg_wkly_wage_chg", "oty_avg_wkly_wage_pct_chg", "avg_monthly_wage")
...
df_avg_rent <- read_excel("Average Rent Prices.xlsx", sheet = 1)
#df_mdn_homevalue = read.xlsx("Median Home Values.xlsx", sheetIndex = 1)
df_mdn_homevalue <- read_excel("Median Home Values.xlsx", sheet = 1)
```
...
modelMonthlyMinWage <- lm(min_monthly_wage ~ Year, data = df_min_wage_without20)
...
```
The minimum monthly wage is highly correlated with the year. The adjusted R-squared is .979 and the R Value is .989. I am surprised that the minimum wage is so highly correlated with year. I thought that it wouldn't be since there isn't a raise each year. However, this does make sense because the minimum wage always increases in each year recorded.
```{r, echo=FALSE}
summary(modelMonthlyMinWage)
```
The following values are the predicted minimum monthly wages in Los Angeles, CA for 2017, 2018, 2019, and 2020 respectively.
```{r, echo=FALSE}
predictMonthlyMinWage

# The final prediction dataframe is created to be included on the plot.
df_predict_min_wage_without20_final <- rbind(df_min_wage_without20_final, data.frame(Year = c(2017:2020), min_monthly_wage_without20 = predictMonthlyMinWage))

# Plot the linear model.

plotMinMonthlyWage <- ggplot() + 
  geom_point(data = df_predict_min_wage_without20_final, aes(x=Year, y=min_monthly_wage_without20), color = "firebrick") +
  geom_point(data = df_min_wage, aes(x=Year, y=min_monthly_wage), color = "black") +
  geom_smooth(data = df_predict_min_wage_without20_final, aes(x=Year, y=min_monthly_wage_without20), color = "red", method="lm") +
  geom_smooth(data = df_min_wage, aes(x=Year, y=min_monthly_wage), color = "blue", method="lm") +
   annotate(geom = "text", label = "Best fit line with $15 an hour increase", 
           x = 2009.7, y = 2360, hjust = 0, 
           colour = "blue") +
  annotate(geom = "text", label = "Best fit line without $15 an hour increase", 
           x = 2009, y = 1160, hjust = 0, 
           colour = "red") +
  ylab("Minimum Monthly Wage") +
  scale_x_continuous(breaks = seq(from=1998, to=2020, by=2)) +
  scale_y_continuous(labels = dollar) +
  theme_bw()
```
The model predicted  that the minimum monthly wage in 2020 would be $1,698.09. I calculated what the hourly wage would be from this amount by dividing it by 4 and then dividing it by 40. According to the model, I found that the minimum wage in Los Angeles would be approximately $10.61 an hour in 2020 if there were no legislation raising it to $15 an hour.
...
```
The predicted average rent prices on a one bedroom apartment for 2017 to 2020 are:
```{r, echo=FALSE, message=FALSE}
predictAvg1bedrent

# Plot the linear model

plotAvg1bedrent <- ggplot(df_avg_rent_1bed,aes(x=Year,y=Avg_Rent)) + 
  geom_point() + 
  #  xlim(2011,2020) +
  stat_smooth(method="lm",fullrange=TRUE) +
  ylab("Average Rent") +
  scale_x_continuous(breaks = seq(from=2011, to=2020, by=1), limits = c(2011, 2020)) +
  scale_y_continuous(labels = dollar) +
  theme_bw()
```
...
```
The average median home value is also highly correlated with the year. The Adjusted R-Squared is .9463 and the R value is .9728.
```{r, echo=FALSE}
...
plotAvgMdnHomeValue <- ggplot(df_avg_mdn_homevalue, aes(x=Year, y=Avg_Median_HomeValue)) + 
  geom_point() + 
  #xlim(2011,2020) +    
  stat_smooth(method="lm",fullrange=TRUE) +
...
```
The monthly mortgage payments were not as correlated with the year as the previously observed items, however the correlation was still very significant. The adjusted R-squared is .8826 and the R Value is .9395. The monthly mortgage payments were likely not as correlated with year because the interest rates for loans vary from year to year. They do not steadily increase like the average median home value or the average rent.
```{r, echo=FALSE}
...
plotMonthlyMortgage <- ggplot(df_avg_mdn_homevalue, aes(x=Year, y=Monthly_Mortgage_Payment)) + 
  geom_point() + 
  #xlim(2011,2020) +    
  stat_smooth(method="lm",fullrange=TRUE) +
...
```
The average monthly wage is significantly correlated with the year. It is the least correlated with year of all the variables I observed. The adjusted R-squared is .8258 and the R Value is .9087.
```{r, echo=FALSE}
...
plotAvgMonthlyWage <- ggplot(df_avg_wage, aes(x=Year, y=avg_monthly_wage)) + 
  geom_point() + 
  #xlim(2010,2020) + 
  stat_smooth(method="lm",fullrange=TRUE) +
  ylab("Average Monthly Wage") +
  scale_x_continuous(breaks = seq(from=2011, to=2020, by=1), limits = c(2011, 2020)) +
  scale_y_continuous(breaks = seq(from=4000, to=5000, by=250), labels = dollar) +
  theme_bw()
```
...
df_all_monthly <-  df_min_wage_without20_final %>%
  full_join(df_avg_wage_final) %>%
  full_join(df_avg_rent_1bed) %>%
  full_join(df_monthly_mortgage[-7,]) %>%
  select(Year, Monthly_Mortgage_Payment, avg_monthly_wage, Avg_Rent, min_monthly_wage_without20) %>%
  gather(Item, Amount, Monthly_Mortgage_Payment:min_monthly_wage_without20, -Year)
```
...
plotAllMonthly <- ggplot(df_all_monthly, aes(x = Year, y = Amount, color = Item)) +
  ...
  scale_x_continuous(breaks = seq(from=2008, to=2016, by=1), limits = c(2008,2016)) +
  scale_y_continuous(labels = dollar) +
  scale_color_discrete("Item", breaks=c("Monthly_Mortgage_Payment","avg_monthly_wage","Avg_Rent", "min_monthly_wage_without20"), labels=c("Mortgage Payment","Average Wage","Average Rent","Minimum Wage")) +
  ylab("Monthly Amount") +
  theme_bw()
...
I again compared all monthly items against year, but this time with the predicted values for 2017 to 2020 from each linear model. I did not use the minimum wage model's prediction for 2020. I used the minimum monthly wage based off the assumed $15 an hour minimum wage in 2020. The data for the minimum wage without the assumed wage is also included for comparison. For the monthly mortgage, I took out Zillow's incomplete prediction for 2017 an opted for the model's prediction for that year.
...
df_predict_all_monthly <-  df_min_wage_final %>%
  ...
  full_join(df_predict_min_wage_without20_final) %>%
  select(Year, Monthly_Mortgage_Payment, avg_monthly_wage, Avg_Rent, min_monthly_wage, min_monthly_wage_without20) %>%
  gather(Item, Amount, Monthly_Mortgage_Payment:min_monthly_wage_without20, -Year)
...
plotPredictAllMonthly <- ggplot(df_predict_all_monthly, aes(x = Year, y = Amount, color = Item)) +
  ...
  geom_point() +
  stat_smooth(fullrange = TRUE) +
  scale_x_continuous(breaks = seq(from=2008, to=2020, by=2), limits = c(2008,2020)) +
  scale_y_continuous(labels = dollar) +
  scale_color_discrete("Item", breaks=c("Monthly_Mortgage_Payment","avg_monthly_wage","Avg_Rent","min_monthly_wage", "min_monthly_wage_without20"), labels=c("Mortgage Payment","Average Wage","Average Rent","Minimum Wage with $15","Minimum Wage w/o $15")) +
  ylab("Monthly Amount") +
  theme_bw()
...
I have data for average rent by each neighborhood in Los Angeles, but only for the most current complete month. For this study that is September 2016. This data removes the most expensive neighborhood that was present in my original rent data. I estimated what the average rent would be in each neighborhood by applying the percent change for the average rent in Los Angeles from 2016 to 2020 to each neighborhood. I could not use prediction with linear models since I did not have more than one month of data. 

The neightborhood data is shown as an average across all rental types, but my project only deals with 1 bedroom apartments. I needed to find out what the average rent difference is between all types and a 1 bedroom apartment in my data. Then I found out what percent this difference is from the price so I can apply it to the neighborhood data.
```{r, echo=FALSE}
#df_neighborhood_avg_rent <- read.xlsx("Neighborhood Average Rent.xlsx", sheetIndex = 1)
df_neighborhood_avg_rent <- read_excel("Neighborhood Average Rent.xlsx", sheet = 1)
colnames(df_neighborhood_avg_rent) <- make.names(colnames(df_neighborhood_avg_rent))
```
```{r, echo=TRUE}
# Find the percent difference between average of all types and average of 1 bedroom.
colnames(df_avg_rent) <- make.names(colnames(df_avg_rent))
...
```
My assumed minimum wage of $15 an hour for 2020 calculates that a full time minimum wage employee will receive $2,400 a month. I used the neighborhood predictions to see which neighborhoods this wage would be able to afford according to a common idea which recommends the cost of a place to live should be 30% of the income. Unfortunately, my predicted data shows that no neighborhoods in Los Angeles will have a one bedroom rent that is 30% of the $2,400 a month earned by the minimum wage. I decided that a minimum wage earner might be willing to deal with less disposable income in order to afford their living situation. If they spent 60% of their earnings that would be $1,440. If there was no minimum wage raise to $15 an hour for 2020 the predicted hourly wage was about $10.61. This would give the minimum wage earner only $1,018.85 to spend on a place according to our budge. Where could a minimum wage earner afford to live if they spent 60% of earnings on a one bedroom apartment, both with and without the minimum wage raise? 

I also wanted to see what neighborhoods the average wage could afford in Los Angeles in 2020. According to my predictions, the average wage would give a person $4,901.60 a month. If this person uses 60% of their income for housing that would mean they spend $2.940.96 a month.

The graph below illustrates what neighborhoods are affordable to the minimum wage earner and the average wage earner when they spend 60% of their income. The top group also shows which neighborhoods neither can afford with that 60%. The affordable neighborhoods for each group are below the horizontal lines.

### Neighborhood Average One Bedroom Rent in 2020
```{r, echo=FALSE,  fig.height=8}
neighb_graph_df <- data_frame(x = df_neighborhood_avg_rent$Neighborhood,  y = df_neighborhood_avg_rent$Avg_Rent_2020)

levels_order <- neighb_graph_df[order(neighb_graph_df$y, decreasing=FALSE),]$x

neighb_graph_df$x <- factor(neighb_graph_df$x, levels = levels_order)

ggplot(neighb_graph_df, aes(x = x, y = y)) +
  geom_point(aes(colour = y)) +
  geom_vline(xintercept = 11.5, colour = "grey33") +
  geom_vline(xintercept = 46.5, colour = "grey33") +
  geom_vline(xintercept = .7, colour = "grey33") +
  scale_color_gradientn(colours = c("red","purple","blue"), 
                        values = rescale(x = c(1000, 1440, 2940, 4500)), 
                        guide = "none") +
  annotate(geom = "text", label = "Affordable on minimum wage with $15/hr raise", 
           x = 11, y = 1560, hjust = 0, 
           colour = "red") +
  annotate(geom = "text", label = "Affordable on average wage", 
           x = 46, y = 2950, hjust = 0, 
           colour = "purple") +
  annotate(geom = "text", label = "Affordable to higher income earners", 
           x = 63, y = 2500, hjust = 0, 
           colour = "blue") +
  annotate(geom = "text", label = "Affordable on minimum wage without $15/hr raise", 
           x = 1.3, y = 1800, hjust = 0, 
           colour = "firebrick") +
  coord_flip() +
  theme_bw() +
  theme(axis.text.y=element_text(size=rel(0.7))) +
  xlab("Neighborhood") +
  ylab("Average Rent Price")
```

No neighborhoods are affordable without the minimum wage raise. This means that a minimum wage earner would likely have to rent a single room or area within another person's apartment or have a room mate to afford costs. The minimum wage increase will allow the minimum wage earner 11 neighborhood choices to afford on their own.

## Future Research

There were a few areas in my project that could be improved upon in further research. The data that was used did not stretch far back in years. Therefore, I could only base my predictions off less than 10 records for each database. The minimum wage data goes back to the early 1900's, but I wanted to use a similar number of years for each item. I would like to see this study done in the future when there is several decades of median home value, average rent, and average income data. My analysis was limited because of my assumption that an increase in the minimum change won't impact the housing market. I could not factor in any changes this would have on the average wage as I assumed it would not see an increase similar to what will happen to the minimum wage.

The average rent data was also influenced by very expensive neighborhoods. If the median rent data for Los Angeles could be found, it would provide a better picture of rent prices. The rent data I got from rentjungle.com also had the problem that the neighborhood's average rents were calculated in a way that included people listing a single room in a home or apartment for rent. Renting a single room in a place usually costs less than renting an entire one bedroom apartment. Therefore, neighborhoods like Central City were priced lower than they should be because of the influence of people listing single rooms in their place. I would like to see the study redone while filtering out these single rooms. Additionally, the listings were for apartments were not as comprehensive as the data from Zillow. I would like to see how the rent prices would be affected if there were a broader list of apartments.

Lastly, the wage data used gross income and not net income after taxes. I had calculated these amounts but did not use them since I was focusing on a minimum wage earner who would likely get a good portion of their taxes back. If taxes were introduced to the data, both types of wage earners would see a decrease in what they can afford. I would like to see how the study would look when considering income after taxes. 
