...
```
The minimum monthly wage is significantly correlated with the year, but it is the least correlated of all items. The adjusted R-squared is .7939 and the R Value is .8910. I expected this to be the case since the minimum wage does not change every year like the average wage or housing prices.
```{r, echo=FALSE}
summary(modelMonthlyMinWage)
```
The predicted minimum monthly wages for 2017 to 2020 are:
```{r, echo=FALSE}
...
scale_x_continuous(breaks = c(seq(from=1998, to=2020, by=2))) +
theme_bw()
```
...
plotAvgMonthlyWage = ggplot(df_avg_wage, aes(x=Year, y=avg_monthly_wage)) + geom_point() + xlim(2010,2020) + 
stat_smooth(method="lm",fullrange=TRUE) +
ylab("Average Monthly Wage") +
scale_x_continuous(breaks = c(seq(from=2012, to=2020, by=1))) +
theme_bw()
```
The model predicted the minimum monthly wage in 2020 would be 2011.455. I calculated what the hourly wage would be from this amount by dividing it by 4 and then dividing it by 40. According to the model, I found that the minimum wage in Los Angeles would be approximately $12.57 an hour by 2020 if there were no legislation raising it to $15 an hour.
...
ggplot(neighb_graph_df, aes(x = x, y = y)) +
   ...
   geom_vline(xintercept = 1.5, colour = "grey33") +
   scale_color_gradientn(colours = c("red","purple","blue"), 
                         values = c(rescale(x = c(1000, 1440, 2940, 4500))), 
                         guide = "none") +
